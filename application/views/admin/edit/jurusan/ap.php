<div class="container-fluid">
    ...
    <div class="card card-body shadow-sm mt-4">
        ...
        ?>
        <?= form_open('admin/jurusan/ap', 'onsubmit="disable()"'); ?>
        <label class="bold">Nama Jurusan</label>
        <input type="text" name="name" class="form-control" placeholder="Nama Jurusan" value="<?= $ap['name']; ?>">
        <?= form_error('name', '<span class="text-danger bold">', '</span>'); ?>
        <div class="pt-3"></div>
        <label class="bold">Konten</label>
        <textarea name="description" class="form-control mb-3" id="summernote"><?= $ap['description']; ?></textarea>
