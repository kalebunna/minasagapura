<div class="container-fluid">
    ...
    <div class="card card-body shadow-sm mt-4">
        ...
        ?>
        <?= form_open_multipart('admin/edit/struktur', 'onsubmit="disable()"'); ?>
        <div class="row">
            <div class="col-lg-6 col-sm-12">
                <label class="bold">Gambar Struktur Organisasi</label>
                <input type="file" name="image" class="form-control">
            </div>
            <div class="col-lg-6 col-sm-12">
                <label class="bold">Deskripsi singkat</label>
                <textarea name="description" class="form-control" placeholder="Deskripsi singkat"><?= $struktur['description']; ?></textarea>
                <?= form_error('description', '<span class="text-danger bold">', '</span>'); ?>
            </div>
        </div>
        <div class="pt-3"></div>
        <label class="bold">Konten</label>
        <textarea name="content" class="form-control mb-3" id="summernote"><?= $struktur['content']; ?></textarea>
