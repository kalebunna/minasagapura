<div class="container-fluid">
    ...
    <div class="card card-body shadow-sm mt-4">
        ...
        <?= form_open('admin/edit/logomotto', 'onsubmit="disable()"'); ?>
        <label class="bold">Deskripsi singkat</label>
        <textarea name="description" class="form-control" placeholder="Deskripsi singkat"><?= $logomotto['description']; ?></textarea>
        <?= form_error('description', '<span class="text-danger bold">', '</span>'); ?>
        <div class="pt-3"></div>
        <label class="bold">Konten</label>
        <textarea name="content" class="form-control mb-3" id="summernote"><?= $logomotto['content']; ?></textarea>
