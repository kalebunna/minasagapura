...
<div class="container-fluid">
    ...
    <div class="card card-body shadow-sm mt-4">
        ...
        ?>
        <?= form_open_multipart('admin/artikel/create', 'onsubmit="disable()"'); ?>
        <label class="bold">Judul</label>
        <input type="text" name="title" class="form-control" value="<?= set_value('title'); ?>" autocomplete="off" autofocus="on">
        <?= form_error('title', '<p class="text-danger bold">', '</p>'); ?>
        <div class="pt-3"></div>
        <label class="bold">Gambar</label>
        <input type="file" name="image" class="form-control">
        <small><i>Disarankan untuk menggunakan gambar ukuran 700x300</i></small>
        <div class="pt-3"></div>
        <label class="bold">Artikel</label>
        <textarea name="article" id="summernote" class="form-control" autocomplete="off"><b>Pematangsiantar, <?= date('Y M d'); ?></b> - <?= set_value('article'); ?></textarea>
